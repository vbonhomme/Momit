...
```
Below `from`, I detailed the main steps that happen behind the curtain.
...
Talking about pipes, all [MomX](https://github.com/MomX) packages use them intensively, so you may want to read [magrittr's vignette](https://cran.r-project.org/package=magrittr/vignettes/magrittr.html)
...
Because once you started to like, you will love it. Particularly if you do some "modern R". If you never heard of it or if you do not think it's worth the pain, let me disagree with you and please read the [dplyr's introduction](https://cran.r-project.org/package=dplyr/vignettes/dplyr.html) (5 min), and [_R for data science_](http://r4ds.had.co.nz/) (from 2 days to 2 years). 
...
library(Momocs)
x
x %$% Out(coo) %>% panel # just a panel plot (ugly but data were dummy)
# equivalent to
# Out(x$coo) %>% panel

Out(x$coo, fac=select(x, -coo))

# or if this is not outlines but landmarks:
Ldk(x$coo, fac=select(x, -coo))
```
