...
#' @param extension `character`, the extension to use (default `mom`)
#'
#' @name write
#' @rdname write
#' @export
...
write_separately <- function(x, folder="", extension=""){
  if (!dir.exists(folder))
    dir.create(folder)
  paths <- paste0(folder, "/", x$name, extension)
  silent <- lapply(seq_along(x), function(i) x[i, ] %>% .mom_df_2_mom %>% writeLines(paths[i]))
}
